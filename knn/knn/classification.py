...
class KNNClassifier:
    ...
    def _predict_precomputed(self, indices, distances):
        answer = np.zeros(indices.shape[0])
        if self._weights == 'uniform':
            for i in range(len(indices)):
                closest_y = self._labels[indices[i]]
                values, counts = np.unique(closest_y, return_counts=True)
                answer[i] = values[counts == counts.max()].min()
        elif self._weights == 'distance':
            for i in range(len(indices)):
                val = {}
                closest_y = self._labels[indices[i]]
                for j in range(len(closest_y)):
                    if val.get(closest_y[j]):
                        val[closest_y[j]] += 1 / (self.EPS + distances[i][j])
                    else:
                        val[closest_y[j]] = 1 / (self.EPS + distances[i][j])
                answer[i] = max(val.items(), key=operator.itemgetter(1))[0]
